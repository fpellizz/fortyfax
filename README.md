...
- **Two-factor authentication (FortiToken/OTP)** for Fortinet: the token is requested in a dialog when the gateway asks for it
...
### Fortinet two-factor authentication (FortiToken / OTP)

If the FortiGate requires a second factor, `openfortivpn` prints a prompt and waits for
the code on standard input. Fortyfax intercepts that prompt and opens a dialog: type the
code generated by the FortiToken app and click **Send**. The connection resumes on its own.

The prompt is waited for at most 3 minutes; after that the connection is closed instead of
leaving a stuck process behind.

**Two-factor options in the profile editor** (Fortinet with username/password only):

- **Disable FTM push**: when the gateway offers FortiToken Mobile *push*, `openfortivpn`
  uses it by default and silently waits for the approval on the phone — no code is asked.
  Enable this option to be asked for the code instead (`--no-ftm-push`).
- **OTP prompt**: the text to look for when the server uses a custom prompt (`--otp-prompt`)
- **OTP send delay**: seconds to wait before sending the code, for servers that need it
  (`--otp-delay`)

> **Realm**: if the gateway is configured with multiple authentication realms, fill in the
> **Realm** field, otherwise the login is rejected before the second factor comes into play.
